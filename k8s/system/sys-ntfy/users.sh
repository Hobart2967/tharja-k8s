...
if echo "$YQ_VERSION" | grep -qi "mikefarah"; then
  CONFIGMAPS=$(kubectl -n "$1" get secrets -o yaml | yq eval '.items
    | map(select(.metadata.annotations."hobart2967.github.io/ntfy-user" == "true"))
    | map({"name": (.data.username | @base64d), "password": .data.password, "role": (.data.role | @base64d), "extras": (.data.extras)})' -)
else
