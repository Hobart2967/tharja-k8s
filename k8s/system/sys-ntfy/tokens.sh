...
if echo "$YQ_VERSION" | grep -qi "mikefarah"; then
  CONFIGMAPS=$(kubectl -n "$1" get secrets -o yaml | yq eval '.items
    | map(select(.metadata.annotations."hobart2967.github.io/ntfy-token" == "true"))
    | map({"name": (.data.username | @base64d), "accessToken": .data.access_token })' -)
else
...
#echo $CONFIGMAPS
echo "$CONFIGMAPS" > .tmp/tokens.yaml